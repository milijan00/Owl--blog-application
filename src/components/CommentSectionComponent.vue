<template>
	<section class="container mx-auto  row p-3 flex-column justify-content-start   " id="comment-section">
			...
			<form method="POST" action="#" name="formCreateComment" @submit.prevent="onSubmit">
				<section class="row justify-content-center">
					<article class="col-11  mb-3">
						<textarea placeholder="What's on Your mind?" rows="2" cols="20" class="form-control" v-model="content"></textarea>
		...
		<section v-if="comments.length" >
			<CommentComponent v-for="c in comments" :key="c" :comment="c" :parentComment="true" @comment-deleted="refresh" @comment-updated="refresh" @reload-comments="refresh" @server-error="serverError" @child-created="refresh"></CommentComponent>	
		</section>
		<article v-else>
			<p class=" ">Nobody has commented yet.</p>
		</article>
		<!-- <CommentsWithFormComponent :comments="comments" @server-error="serverError" @refresh-comments="refresh" @hide-comments="hide"></CommentsWithFormComponent> -->
		<!-- END -->
	</section>
</template>
