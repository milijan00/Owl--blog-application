...
namespace BlogApp.Implementations
{
    public class UseCaseHandler
    {
        ...
        public void HandleCommand<TRequest>(ICommand<TRequest> command, TRequest data)
        {
            try
            {
                this.HandleAuthentication(command);
        ...
        public TResponse HandleQuery<TRequest, TResponse>(IQuery<TRequest, TResponse> query, TRequest data)
        {
            try
            {
                this.HandleAuthentication(query);
        ...
        private void HandleAuthentication(IUseCase usecase)
        {
            if(usecase.Name != "EfCreateUserCommand" && this._actionUser.Id == 0)
            {
        ...
        private void HandleLoggingAndAuthorization<TRequest>(IUseCase useCase, TRequest data )
        {
            ...
            if (useCase.AdminOnly && this._actionUser.Identity != "Administrator")
            {
                throw new UnauthorizedAccessException();
            }
        }
